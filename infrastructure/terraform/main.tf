    ...
    widgets = [
      ...
      },
      {
        ...
        properties = {
          ...
          region = var.aws_region
          query  = "SOURCE '${var.cloudwatch_log_group_prefix}/kubernetes' | fields @timestamp, @message | filter @message like /ERROR|WARN/ | sort @timestamp desc | limit 50"
          view   = "table"
        }
      },
      {
        ...
        properties = {
          ...
          region = var.aws_region
          query  = "SOURCE '${var.cloudwatch_log_group_prefix}/kubernetes' | fields @timestamp, @message | filter @message like /MicroCRMApplication|Liquibase|liquibase/ | sort @timestamp desc | limit 50"
          view   = "table"
        }
      }
    ]
