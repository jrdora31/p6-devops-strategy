...
| Le code et les images ne sont pas encore analysés automatiquement. | Les stages `quality` et `release` ne contiennent aucun job SonarQube ou Trivy. | Une faille peut être découverte après la publication. | Ajouter SonarQube avant le build et Trivy avant la publication. | À faire |
