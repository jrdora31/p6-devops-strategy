...
- **Cas d’usage MicroCRM :** scanner les images frontend et backend après leur build et avant leur publication ou leur déploiement.
...
- **Description :** distribution Kubernetes légère, installable sur une machine Linux et adaptée à un environnement contraint en ressources.
- **Catégorisation :** orchestration de conteneurs.
- **Fonctionnalités clés :** API Kubernetes, exécution de Pods, Services, Deployments et intégration avec Helm, avec plusieurs composants fournis par défaut.
- **Cas d’usage MicroCRM :** exécuter un POC Kubernetes sur une seule instance EC2 afin de valider le déploiement cloud et les manifests sans supporter le coût d’un control plane managé.
- **Contraintes et coût :** K3s n’ajoute pas de licence ni de coût de control plane, mais l’instance EC2, le stockage, l’adresse IPv4 et le trafic peuvent être facturés. Sur un seul nœud, la solution ne fournit ni haute disponibilité, ni élasticité automatique de l’infrastructure.
- **Références :** K3s, [Requirements](https://docs.k3s.io/installation/requirements), consulté le 16 août 2026 ; AWS, [Track your Free Tier usage for Amazon EC2](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ec2-free-tier-usage.html), consulté le 16 août 2026.

#### 2.2.13 Amazon EKS

- **Description :** service Kubernetes managé par AWS, qui prend en charge le control plane du cluster.
- **Catégorisation :** orchestration de conteneurs.
- **Fonctionnalités clés :** control plane managé, intégration IAM/VPC, nœuds EC2 ou Fargate et options de mise à l’échelle.
- **Cas d’usage MicroCRM :** cible possible pour une architecture de production distribuée, lorsque la disponibilité, la montée en charge et l’exploitation managée justifient son coût.
- **Contraintes et coût :** chaque cluster EKS est facturé à l’heure, en plus des nœuds, volumes, adresses IPv4 et transferts. Au tarif standard consulté, le control plane coûte 0,10 USD par heure : ce choix est disproportionné pour le POC à budget strict.
- **Références :** AWS, [Amazon EKS pricing](https://aws.amazon.com/eks/pricing/) et [Amazon EKS compute](https://docs.aws.amazon.com/eks/latest/userguide/eks-compute.html), consultés le 16 août 2026.

#### 2.2.14 Liquibase

- **Description :** outil de gestion et de versionnement des évolutions de schéma de base de données.
- **Catégorisation :** migrations de base de données.
- **Fonctionnalités clés :** changelogs versionnés, changesets, formats YAML/XML/JSON/SQL, historique d’exécution et mécanismes de rollback selon les changements.
- **Cas d’usage MicroCRM :** Liquibase est déjà présent dans le backend initial et définit un schéma commun à HSQLDB pour les tests et PostgreSQL pour l’environnement déployé, depuis un changelog YAML intégré à Spring Boot. Aucune évolution de schéma n’est prévue dans le périmètre DevOps de ce projet ; l’outil n’est donc ni ajouté ni étendu.
- **Contraintes et coût :** les changements doivent être conçus et testés ; certains rollbacks exigent une définition manuelle et ne remplacent jamais une sauvegarde des données.
- **Références :** Liquibase, [What is a changelog?](https://docs.liquibase.com/pro/user-guide-4-33/what-is-a-changelog), [YAML changelog example](https://docs.liquibase.com/concepts/changelogs/yaml-format.html) et [Rollback commands](https://docs.liquibase.com/community/reference-guide-5-0/init-update-and-rollback-commands/what-are-rollback-commands), consultés le 16 août 2026.

#### 2.2.15 Flyway

- **Description :** outil de migration de base de données fondé principalement sur des scripts versionnés.
- **Catégorisation :** migrations de base de données.
- **Fonctionnalités clés :** migrations versionnées et répétables, table d’historique, validation et intégration aux applications Java et aux pipelines.
- **Cas d’usage MicroCRM :** appliquer des scripts SQL simples et ordonnés sur PostgreSQL au démarrage ou pendant le déploiement.
- **Contraintes et coût :** l’approche SQL est directe mais plus dépendante du moteur ; les fonctions d’undo ne sont pas toutes disponibles dans l’édition Community et une stratégie de sauvegarde reste nécessaire.
- **Références :** Redgate, [Migrations-based approach](https://documentation.red-gate.com/fd/migrations-based-approach-168984769.html), [Rollback strategy](https://documentation.red-gate.com/fd/implementing-a-roll-back-strategy-138347142.html) et [Frequently asked questions](https://documentation.red-gate.com/flyway/reference/usage/frequently-asked-questions), consultés le 16 août 2026.
...
#### 2.3.5 Orchestration Kubernetes sur AWS

| Comparé à | K3s sur EC2 | Amazon EKS |
|---|---|---|
| K3s sur EC2 | — | Par rapport à K3s, **Amazon EKS** fournit un control plane managé et convient mieux à une cible multi-nœuds. En revanche, il ajoute un coût fixe de cluster et les coûts des ressources AWS. |
| Amazon EKS | Par rapport à Amazon EKS, **K3s** évite le coût du control plane et convient au POC sur une EC2. En revanche, l’exploitation et la disponibilité du cluster restent à la charge du projet. | — |

K3s est retenu pour le POC à coût contenu. EKS reste une option si une architecture multi-nœuds devient nécessaire.

#### 2.3.6 Versionnement du schéma

| Comparé à | Liquibase | Flyway | Scripts SQL sans outil dédié |
|---|---|---|---|
| Liquibase | — | Par rapport à Liquibase, **Flyway** privilégie des migrations SQL simples. En revanche, il est moins adapté à un schéma structuré commun à HSQLDB et PostgreSQL. | Par rapport aux scripts seuls, **Liquibase** conserve un historique et des changesets. En revanche, il ajoute une dépendance et une convention de changelog. |
| Flyway | Par rapport à Flyway, **Liquibase** propose des changelogs YAML et des changements plus portables. En revanche, son modèle est plus riche à apprendre. | — | Par rapport aux scripts seuls, **Flyway** apporte ordre et historique des migrations. En revanche, les scripts SQL restent plus dépendants du moteur. |
| Scripts SQL sans outil dédié | Par rapport à Liquibase, **les scripts seuls** ne demandent pas d’outil supplémentaire. En revanche, l’ordre, l’historique et le contrôle d’exécution sont à développer. | Par rapport à Flyway, **les scripts seuls** sont immédiats à écrire. En revanche, ils ne fournissent pas de traçabilité ni de validation standard. | — |

Liquibase est déjà présent dans le backend ; il est retenu et doit être maintenu. Le rollback de schéma ne remplace pas une restauration de données.

#### 2.3.7 Stratégie de déploiement applicatif

| Comparé à | Rolling update | Blue-green | Canary |
|---|---|---|---|
| Rolling update | — | Par rapport au rolling update, **blue-green** permet une bascule rapide vers un environnement complet. En revanche, il exige presque deux fois plus de capacité. | Par rapport au rolling update, **canary** limite l’exposition d’une version défaillante. En revanche, il exige un routage pondéré et des métriques. |
...
L’audit ne couvre pas de runner GitLab distant, d’environnement AWS, de test de charge ou d’indicateur de production. Les sondages sont des sources déclaratives et les sorties brutes de certains contrôles locaux n’ont pas toutes été conservées ; ces limites bornent le niveau de preuve sans modifier les constats observables au commit audité.
...
| Workflow | [Source Mermaid modifiable](assets/diagrammes/workflow_ci_actuel.md) et [export SVG](assets/diagrammes/workflow_ci_actuel.svg) |
| Tests frontend | [Capture Karma — 8 tests réussis](assets/preuves/tests_frontend_baseline_2026-07-19.png), preuve locale antérieure à la date de synthèse de l’audit |

Le workflow distingue les faits observés dans le repository du flux Docker manuel déclaré par les Ops.

![Workflow CI actuel de MicroCRM](assets/diagrammes/workflow_ci_actuel.svg)
...
- **Description :** versionner les évolutions du schéma si PostgreSQL évolue dans un futur périmètre applicatif.
- **Raisonnement :** aucune évolution de schéma n’est réalisée dans ce projet DevOps. Liquibase est néanmoins déjà intégré au backend initial. Dans un environnement de production, une base persistante non immuable doit évoluer de manière reproductible avec l’application : Liquibase est alors préférable à Flyway pour ses changelogs YAML structurés et plus portables entre HSQLDB et PostgreSQL ; des scripts sans outil dédié offriraient moins de traçabilité.
- **Implémentation :** ne pas ajouter de changeset pour ce projet tant que le schéma ne change pas. Lors d’une évolution applicative future, versionner le changelog Liquibase dans le repository et l’exécuter au démarrage ; tester la compatibilité entre releases, les préconditions et les rollbacks lorsqu’ils sont sûrs. Maintenir séparément les sauvegardes et restaurations de données.
- **Impacts :** ne pas élargir le périmètre actuel au code applicatif, tout en conservant une recommandation exploitable pour appliquer le même schéma dans chaque environnement et limiter les écarts entre releases.

#### 4.2.13 Définir une stratégie de déploiement et de rollback
...
- **Implémentation :** utiliser un `RollingUpdate` Kubernetes avec probes, contrôle HTTP et rollback Helm pour le POC. Préparer le canary comme cible de production avec versions stable/canary, load balancer, paliers `0/5/25/50/100`, métriques et gates ; écarter blue-green tant que le doublement de capacité n’est pas justifié.
- **Impacts :** maîtriser le risque avec les ressources actuelles, puis limiter progressivement l’exposition d’une nouvelle version lorsque l’architecture permettra un vrai canary.

#### 4.2.14 Utiliser K3s pour le POC AWS

- **Description :** déployer le POC Kubernetes avec K3s sur une instance EC2, tout en conservant EKS comme option d’évolution et non comme prérequis.
- **Raisonnement :** AWS fournit la cible cloud et le chemin de montée en charge souhaités, tandis que K3s réduit les ressources et évite le coût fixe du control plane EKS pendant le POC.
- **Implémentation :** provisionner une instance dimensionnée et éligible selon le compte, installer K3s de manière automatisée, déployer avec Helm, surveiller la consommation et détruire les ressources hors période utile. Configurer budget et alertes de facturation avant le déploiement.
- **Impacts :** valider l’orchestration et l’automatisation sur AWS avec un coût contenu, sans présenter le nœud unique comme une architecture haute disponibilité ou automatiquement scalable.
...
Cet ordre suit les dépendances techniques : chaque étape produit les éléments nécessaires à la suivante. La chaîne est d’abord conçue et testée, puis sécurisée et versionnée avant tout déploiement sur AWS.
