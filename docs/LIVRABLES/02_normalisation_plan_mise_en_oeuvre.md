...
Cette partie formalise les règles retenues en `F`, puis vérifiées dans la chaîne CI/CD actuelle.
...
- automatiser les contrôles reproductibles dans GitLab CI ;
- construire une seule fois puis réutiliser des artifacts et images traçables ;
- échouer tôt sur un contrôle bloquant, tout en conservant les rapports ;
- externaliser les secrets et appliquer des droits minimaux ;
- privilégier une solution simple, testable et documentée (`KISS`, `DRY`, `YAGNI`).
...
Le code respecte les conventions propres à Angular/TypeScript, Java/Spring Boot, Python et Bash. Les noms doivent être explicites, les responsabilités séparées et la duplication évitée. Les contrôles automatisés incluent les tests, SonarQube et ShellCheck ; toute exception significative doit être justifiée dans le code ou la documentation.
...
`dev` est la branche d’intégration et `main` la branche principale. Les changements passent par une merge request. Les releases utilisent des tags SemVer (`vMAJEUR.MINEUR.CORRECTIF`) ; les images techniques portent aussi le SHA du commit et le manifeste de release conserve les digests.
...
Chaque changement destiné à `dev` ou `main` passe par une merge request relue. L’acceptation exige une portée claire, une CI réussie, l’absence de secret, des rapports exploitables et une documentation mise à jour si le comportement change. La fusion est refusée tant qu’un contrôle bloquant échoue ou qu’un risque critique corrigible reste ouvert.
...
Les pipelines sont créés pour les merge requests, `dev`, `main` et les tags. Avec le plan SonarQube Cloud Free, l’analyse SonarQube s’exécute uniquement sur les merge requests et sur `main` : les push directs sur `dev` conservent les tests, scans et publication, mais le déploiement staging est réservé à une pipeline Web autorisée qui provisionne l’infrastructure AWS éphémère. Les changements destinés à être intégrés doivent donc passer par une merge request.
...
| `dev` | mêmes contrôles, scans/publication ; déploiement staging uniquement via pipeline Web autorisée |
...
Les secrets sont fournis par des variables GitLab protégées et masquées ou par des identités temporaires AWS OIDC ; ils ne sont pas codés dans le dépôt. Trivy contrôle le repository, l’IaC, Kubernetes et les images, y compris les secrets embarqués. SonarQube applique son quality gate. Les vulnérabilités `CRITICAL` corrigibles et les secrets détectés bloquent la chaîne ; les rapports sont conservés pour analyse.
...
Le README sert de point d’entrée. La documentation maintenue décrit l’architecture, la CI/CD, la configuration GitLab/AWS, le déploiement, l’exploitation, la sécurité et la performance. Toute modification fonctionnelle ou opérationnelle doit mettre à jour le document concerné et fournir des commandes reproductibles sans secret. Une preuve observée reste distinguée d’une fonctionnalité seulement prévue.
